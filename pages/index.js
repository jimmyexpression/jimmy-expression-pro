...
const SEED_APTS = [
  {id:"a1",cid:"c1",sid:"s1",tid:"t1",date:TODAY,time:"09:00",end:"11:00",status:"confirmed",notes:"Mechas adicionales",dep:0,paid:false},
  {id:"a2",cid:"c2",sid:"s2",tid:"t2",date:TODAY,time:"10:00",end:"12:30",status:"confirmed",notes:"",dep:100000,paid:false},
  {id:"a3",cid:"c3",sid:"s5",tid:"t1",date:TODAY,time:"13:00",end:"14:00",status:"pending",notes:"Primera vez",dep:0,paid:false},
  {id:"a4",cid:"c4",sid:"s3",tid:"t3",date:TODAY,time:"15:00",end:"16:30",status:"confirmed",notes:"",dep:0,paid:false},
  {id:"a5",cid:"c5",sid:"s7",tid:"t2",date:D0(1),time:"10:00",end:"11:30",status:"confirmed",notes:"Turista",dep:0,paid:false},
  {id:"a6",cid:"c1",sid:"s6",tid:"t1",date:D0(2),time:"11:00",end:"12:00",status:"confirmed",notes:"",dep:0,paid:false},
  {id:"a7",cid:"c4",sid:"s10",tid:"t2",date:D0(-1),time:"09:00",end:"10:30",status:"completed",notes:"",dep:0,paid:true},
  {id:"a8",cid:"c3",sid:"s9",tid:"t3",date:D0(-1),time:"11:00",end:"12:00",status:"completed",notes:"",dep:0,paid:true},
];
const SEED_SALES = [
  {id:"v1",date:D0(-1),cid:"c4",sid:"s10",tid:"t2",amount:130000,disc:0,pay:"nequi",tip:20000,type:"service"},
  {id:"v2",date:D0(-1),cid:"c3",sid:"s9",tid:"t3",amount:80000,disc:0,pay:"efectivo",tip:10000,type:"service"},
  {id:"v3",date:D0(-1),cid:null,sid:null,tid:null,amount:85000,disc:0,pay:"tarjeta",tip:0,type:"retail",notes:"Kit Mantenimiento"},
];
const SEED_ORDERS = [
...
// ── DASHBOARD ──────────────────────────────────────────────────────
function Dashboard({apts,sales,clients,inv,svcs,staff,orders}){
  const todayA=apts.filter(a=>a.date===TODAY);
  const todayS=sales.filter(s=>s.date===TODAY);
  const rev=todayS.reduce((a,b)=>a+b.amount+b.tip,0);
  const low=inv.filter(i=>i.stock<=i.min);
  const pendOrd=orders.filter(o=>o.status==="pending").length;
  const nextApts=[...apts].filter(a=>a.date>=TODAY&&a.status!=="cancelled").sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time)).slice(0,5);
  const births=clients.filter(c=>{if(!c.bday)return false;const b=new Date(c.bday),t=new Date(TODAY);return b.getMonth()===t.getMonth()&&Math.abs(b.getDate()-t.getDate())<=4;});
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,marginBottom:18}}>
        <KPI label="Citas hoy" value={todayA.length} sub={`${todayA.filter(a=>a.status==="confirmed").length} confirmadas`} accent={C.goldL} icon="📅"/>
        <KPI label="Ingresos hoy" value={fmtM(rev)} sub="servicios+propinas" accent={C.green} icon="💵"/>
        <KPI label="Ingresos jun." value={fmtM(HIST[5].rev)} sub="+6.9% vs mayo" accent={C.gold} icon="📈"/>
        <KPI label="Pedidos online" value={pendOrd} sub="sin procesar" accent={pendOrd>0?C.orange:C.muted} icon="🛍️"/>
        <KPI label="Stock crítico" value={low.length} sub="por reponer" accent={low.length>0?C.red:C.muted} icon="⚠️"/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:14,marginBottom:14}}>
        <div style={{...sx.card,padding:18}}>
          <div style={{color:C.muted,fontSize:10,letterSpacing:1.5,textTransform:"uppercase",marginBottom:14}}>Próximas citas</div>
          {nextApts.length===0&&<div style={{color:C.muted,padding:20,textAlign:"center"}}>Sin citas próximas</div>}
          {nextApts.map((apt,i)=>{
            ...
            return(
              <div key={apt.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:i<nextApts.length-1?`1px solid ${C.border}11`:"none"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:C.cream,fontWeight:600,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cli?.name||"—"}</div>
                  <div style={{color:C.muted,fontSize:11}}>{svc?.name}{svc?.duration&&<span style={{color:C.muted}}> · ⏱{svc.duration}min</span>} · <span style={{color:st?.color}}>{st?.name}</span></div>
                </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {low.length>0&&<div style={{...sx.card,padding:14,border:`1px solid ${C.red}44`,background:C.redD}}>
            <div style={{color:C.red,fontWeight:700,fontSize:12,marginBottom:8}}>⚠ Stock crítico</div>
            {low.slice(0,3).map(i=><div key={i.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 0"}}><span style={{color:C.cream,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:130}}>{i.name}</span><span style={{color:C.red,fontWeight:700,flexShrink:0}}>{i.stock}/{i.min}</span></div>)}
          </div>}
          {births.length>0&&<div style={{...sx.card,padding:14,border:`1px solid ${C.rose}44`,background:C.roseD}}>
            <div style={{color:C.rose,fontWeight:700,fontSize:12,marginBottom:6}}>🎂 Cumpleaños</div>
            {births.map(c=><div key={c.id} style={{color:C.cream,fontSize:12}}>{c.name}</div>)}
          </div>}
          <div style={{...sx.card,padding:14}}>
            <div style={{color:C.muted,fontSize:10,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10}}>Ingresos 2025</div>
            <BarM data={HIST} ky="rev" color={C.gold}/>
          </div>
          <div style={{...sx.card,padding:14}}>
            <div style={{color:C.muted,fontSize:10,letterSpacing:1.5,textTransform:"uppercase",marginBottom:8}}>Staff activo</div>
            {staff.filter(s=>s.active).map(st=>(
              <div key={st.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                <Ava name={st.name} color={st.color} size={26}/>
                <div><div style={{color:C.cream,fontSize:12,fontWeight:600}}>{st.name}</div><div style={{color:C.muted,fontSize:10}}>{apts.filter(a=>a.date===TODAY&&a.tid===st.id).length} citas hoy</div></div>
              </div>
            ))}
          </div>
        </div>
      </div>
      {pendOrd>0&&<div style={{...sx.card,padding:14,border:`1px solid ${C.orange}44`,background:C.orangeD}}>
        <div style={{color:C.orange,fontWeight:700,marginBottom:4}}>🛍️ {pendOrd} pedido(s) de marketplace pendiente(s) de procesar</div>
        <div style={{color:C.muted,fontSize:12}}>Ve a Marketplace → Pedidos para gestionarlos</div>
      </div>}
    </div>
  );
}
    ...
    <Modal title={type==="edit"?"Editar cita":"Nueva cita"} onClose={onClose} w={640}>
      ...
      <div style={{display:"flex",gap:8,marginTop:18,flexWrap:"wrap"}}>
        ...
        {f.date&&f.time&&(f.sids||[]).length>0&&(
          <button style={{...sx.ghost,color:C.blue,borderColor:C.blue+"44"}} onClick={()=>window.open(gcalUrl(),"_blank")}>
...
// ── CLIENTES ───────────────────────────────────────────────────────
function Clientes({clients,setClients,apts,sales,svcs,toast}){
  const [q,setQ]=useState("");
  const [tag,setTag]=useState("all");
  const [modal,setModal]=useState(null);
  const allTags=["all",...new Set(clients.flatMap(c=>c.tags||[]))];
  const list=clients.filter(c=>{
    const ms=c.name.toLowerCase().includes(q.toLowerCase())||c.phone?.includes(q)||c.email?.toLowerCase().includes(q.toLowerCase());
    const mt=tag==="all"||(c.tags||[]).includes(tag);
    return ms&&mt;
  });

  function Detail({cli}){
    const ca=apts.filter(a=>a.cid===cli.id).sort((a,b)=>(b.date+b.time).localeCompare(a.date+a.time));
    return(
      <div>
        <div style={{display:"flex",gap:14,marginBottom:18,alignItems:"center"}}>
          <Ava name={cli.name} color={C.gold} size={52}/>
          <div style={{flex:1}}>
            <div style={{color:C.cream,fontWeight:800,fontSize:18}}>{cli.name}</div>
            <div style={{color:C.muted,fontSize:12,marginTop:2}}>{cli.phone} · {cli.email}</div>
            <div style={{display:"flex",gap:5,marginTop:6,flexWrap:"wrap"}}>{(cli.tags||[]).map(t=><Badge key={t} c={C.gold} sm>{t}</Badge>)}<Badge c={C.purple} sm>{cli.src}</Badge></div>
          </div>
          <div style={{textAlign:"right"}}><div style={{color:C.gold,fontWeight:800,fontSize:20}}>{cli.pts} pts</div><div style={{color:C.muted,fontSize:10}}>fidelidad</div></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
          {[{l:"Total",v:fmt(cli.spent),c:C.goldL},{l:"Visitas",v:cli.visits,c:C.rose},{l:"Última visita",v:cli.lastVisit||"—",c:C.blue}].map(k=>(
            <div key={k.l} style={{...sx.card,padding:12,textAlign:"center"}}><div style={{color:k.c,fontWeight:800,fontSize:15}}>{k.v}</div><div style={{color:C.muted,fontSize:10,marginTop:2}}>{k.l}</div></div>
          ))}
        </div>
        {cli.notes&&<div style={{...sx.card,padding:12,marginBottom:14,background:C.surface}}><div style={{color:C.muted,fontSize:10,textTransform:"uppercase",letterSpacing:1,marginBottom:5}}>Notas / Alergias</div><div style={{color:C.cream,fontSize:13}}>{cli.notes}</div></div>}
        <div style={{color:C.muted,fontSize:10,textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Historial</div>
        {ca.slice(0,5).map(a=>{const svc=svcs.find(s=>s.id===a.sid);const sc=STATUS[a.status]||STATUS.pending;return(
          <div key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
            <div><div style={{color:C.cream,fontSize:13}}>{svc?.name}</div><div style={{color:C.muted,fontSize:11}}>{a.date} · {a.time}</div></div>
            <div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{color:C.goldL,fontWeight:700}}>{fmt(svc?.price)}</span><Badge c={sc.c} bg={sc.b} sm>{sc.l}</Badge></div>
          </div>
        );})}
  ...
  function Form({cli}){
    const blank={name:"",phone:"",email:"",bday:"",notes:"",src:"Instagram",tags:[],pts:0,visits:0,spent:0,lastVisit:""};
    ...
    return(
      <Modal title={cli?"Editar cliente":"Nuevo cliente"} onClose={()=>setModal(null)}>
        <Row><Fld lbl="Nombre *"><Inp value={f.name} onChange={e=>set("name",e.target.value)} placeholder="Nombre completo"/></Fld><Fld lbl="Teléfono"><Inp value={f.phone} onChange={e=>set("phone",e.target.value)} placeholder="+57 300..."/></Fld></Row>
        <Row><Fld lbl="Email"><Inp value={f.email} onChange={e=>set("email",e.target.value)} type="email"/></Fld><Fld lbl="Cumpleaños"><Inp type="date" value={f.bday} onChange={e=>set("bday",e.target.value)}/></Fld></Row>
        <Row><Fld lbl="Cómo nos conoció"><Sel value={f.src} onChange={e=>set("src",e.target.value)}>{["Instagram","TikTok","Google","Referido","Hotel","Facebook","Otro"].map(s=><option key={s}>{s}</option>)}</Sel></Fld><Fld lbl="Puntos fidelidad"><Inp type="number" value={f.pts} onChange={e=>set("pts",+e.target.value)}/></Fld></Row>
        <Fld lbl="Notas / Alergias"><Txta value={f.notes} onChange={e=>set("notes",e.target.value)} placeholder="Alergias, colores preferidos..."/></Fld>
        <Fld lbl="Etiquetas">
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:7}}>{(f.tags||[]).map(t=><span key={t} style={{background:C.goldD,color:C.gold,borderRadius:4,padding:"2px 8px",fontSize:12,cursor:"pointer"}} onClick={()=>setF(p=>({...p,tags:p.tags.filter(x=>x!==t)}))}>{t} ×</span>)}</div>
          <div style={{display:"flex",gap:6}}><Inp value={ti} onChange={e=>setTi(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTag()} placeholder="Nueva etiqueta + Enter" style={{flex:1}}/><button style={{...sx.ghost,padding:"9px 12px"}} onClick={addTag}>+</button></div>
        </Fld>
        <div style={{display:"flex",gap:8,marginTop:18}}>
          <button style={sx.btn} onClick={save}>{cli?"Guardar":"Crear"}</button>
  ...
  return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",justifyContent:"space-between"}}>
        <Inp value={q} onChange={e=>setQ(e.target.value)} placeholder="🔍 Nombre, teléfono o email..." style={{flex:1,minWidth:200}}/>
        <div style={{display:"flex",gap:6}}>
          <Sel value={tag} onChange={e=>setTag(e.target.value)} style={{width:160}}>{allTags.map(t=><option key={t} value={t}>{t==="all"?"Todas las etiquetas":t}</option>)}</Sel>
          <button style={sx.btn} onClick={()=>setModal({type:"form",cli:null})}>+ Nuevo cliente</button>
        </div>
      </div>
      <div style={{...sx.card,display:"grid",gridTemplateColumns:"2fr 1.2fr 1.5fr 0.6fr 1fr 0.8fr 1fr",padding:"8px 0",marginBottom:8}}>
        {["Cliente","Teléfono","Etiquetas","Visitas","Total","Puntos","Última visita"].map(h=><div key={h} style={{color:C.muted,fontSize:10,letterSpacing:1.2,textTransform:"uppercase",padding:"0 14px"}}>{h}</div>)}
      </div>
      {list.map(c=>(
        <div key={c.id} onClick={()=>setModal({type:"detail",cli:c})} style={{...sx.card,display:"grid",gridTemplateColumns:"2fr 1.2fr 1.5fr 0.6fr 1fr 0.8fr 1fr",alignItems:"center",marginBottom:6,padding:"11px 0",cursor:"pointer",transition:"background .15s"}} onMouseEnter={e=>e.currentTarget.style.background=C.hover} onMouseLeave={e=>e.currentTarget.style.background=C.card}>
          <div style={{padding:"0 14px",display:"flex",alignItems:"center",gap:8}}><Ava name={c.name} color={C.gold} size={30}/><div><div style={{color:C.cream,fontWeight:600,fontSize:13}}>{c.name}</div><div style={{color:C.muted,fontSize:10}}>{c.src}</div></div></div>
          <div style={{padding:"0 14px",color:C.muted,fontSize:12}}>{c.phone}</div>
          <div style={{padding:"0 14px",display:"flex",gap:4,flexWrap:"wrap"}}>{(c.tags||[]).slice(0,2).map(t=><Badge key={t} c={C.gold} sm>{t}</Badge>)}</div>
          <div style={{padding:"0 14px",color:C.cream,fontSize:13,textAlign:"center"}}>{c.visits}</div>
          <div style={{padding:"0 14px",color:C.goldL,fontWeight:700,fontSize:13}}>{fmtM(c.spent)}</div>
          <div style={{padding:"0 14px",color:C.purple,fontWeight:700,fontSize:13}}>{c.pts}</div>
          <div style={{padding:"0 14px",color:C.muted,fontSize:11}}>{c.lastVisit||"—"}</div>
        </div>
      ))}
      <div style={{color:C.muted,fontSize:11,marginTop:8}}>{list.length} clientes</div>
      {modal?.type==="detail"&&<Modal title="Ficha del cliente" onClose={()=>setModal(null)} w={620}><Detail cli={modal.cli}/></Modal>}
...
function CajaSection({sales,setSales,apts,clients,svcs,staff,toast,empresa,nextFactura}){
  ...
  const dayApts=apts.filter(a=>a.date===date&&(a.status==="completed"||a.paid));
  const aptSales=dayApts.map(a=>{const svc=svcs.find(s=>s.id===a.sid);return{id:"apt_"+a.id,type:"service",date:a.date,cid:a.cid,sid:a.sid,tid:a.tid,amount:svc?.price||0,disc:0,pay:"efectivo",tip:0,notes:"",fromApt:true};});
  const manSales=sales.filter(s=>s.date===date);
  const all=[...aptSales,...manSales];
  // Función de cálculo de valor real de cada venta (soporta pago mixto y descuento en $ o %)
  const calcVal=(s)=>{
    if(s.total) return s.total; // ya calculado al registrar
    const disc=s.discType==="monto"?(s.disc||0):(s.amount*(s.disc||0)/100);
    return Math.max(0,s.amount-disc)+(s.tip||0);
  };
  const totSvc=all.filter(s=>s.type==="service").reduce((a,b)=>a+calcVal(b),0);
  const totRet=all.filter(s=>s.type==="retail").reduce((a,b)=>a+calcVal(b),0);
  const totTip=all.reduce((a,b)=>a+(b.tip||0),0);
  const total=totSvc+totRet;
  // Desglose por método de pago (soporta mixto)
  const byPay=all.reduce((acc,s)=>{
    const v=calcVal(s);
    if(s.pagos&&s.pagos.length>0){
      s.pagos.forEach(pg=>{acc[pg.metodo]=(acc[pg.metodo]||0)+(+pg.monto||0);});
    } else {
  ...
  function SaleModal(){
    ...
    const save=()=>{
      ...
      setSales(p=>[...p,{...f,id:uid(),amount:+f.amount,disc:descMonto,discType:f.discType,discVal:+f.disc,tip:+f.tip,pay:payStr,pagos:f.pagos,total:net}]);
  ...
  return(
    <div>
      ...
      {tab2==="historial"&&(
        <div>
          <div style={{...sx.card,padding:18,marginBottom:14}}><div style={{color:C.muted,fontSize:10,textTransform:"uppercase",letterSpacing:1.5,marginBottom:10}}>Ingresos por mes</div><BarM data={HIST} ky="rev" color={C.gold}/></div>
          {HIST.slice().reverse().map(m=>(
            <div key={m.m} style={{...sx.card,padding:"13px 18px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{color:C.cream,fontWeight:700}}>{m.m} 2025</div>
              <div style={{display:"flex",gap:22}}>
                {[{v:fmt(m.rev),l:"Servicios",c:C.goldL},{v:fmt(m.retail),l:"Retail",c:C.purple},{v:m.cli,l:"Clientes",c:C.rose}].map(k=>(
                  <div key={k.l} style={{textAlign:"center"}}><div style={{color:k.c,fontWeight:700}}>{k.v}</div><div style={{color:C.muted,fontSize:10}}>{k.l}</div></div>
                ))}
              </div>
            </div>
          ))}
...
function Nomina({staff,setStaff,apts,sales,svcs,toast}){
  ...
  const calc=(m)=>{
    ...
    const mSales=sales.filter(s=>s.tid===m.id&&enRango(s.date));
    const totalSales=[...mApts.map(a=>svcs.find(s=>s.id===a.sid)?.price||0),...mSales.map(s=>s.amount)].reduce((a,b)=>a+b,0);
    const commission=totalSales*(m.comm/100);
    ...
    return{totalSales,commission,net,healthE,pensionE,healthR,pensionR,paraf,total:net+healthR+pensionR+paraf};
  };
  ...
  function StaffForm({member}){
    ...
    return(
      <Modal title={member?"Editar empleado":"Nuevo empleado"} onClose={()=>setModal(null)}>
        ...
        <Row><Fld lbl="Salario base"><Inp type="number" value={f.base} onChange={e=>set("base",e.target.value)}/></Fld><Fld lbl="Comisión %"><Inp type="number" value={f.comm} onChange={e=>set("comm",e.target.value)}/></Fld></Row>
          ...
          return(
            <div key={member.id} style={{...sx.card,padding:20,marginBottom:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10}}>
                {[
                  {l:"Salario base",v:fmt(p.net-p.commission+p.healthE+p.pensionE),c:C.cream},
                  {l:"Ventas mes",v:fmt(p.totalSales),c:C.blue},
                  {l:"Comisión",v:fmt(p.commission),c:C.green},
  ...
  function ItemForm({item}){
    ...
    return(
      <Modal title={item?"Editar producto":"Nuevo producto"} onClose={()=>setModal(null)}>
        <Row><Fld lbl="Nombre *"><Inp value={f.name} onChange={e=>set("name",e.target.value)}/></Fld><Fld lbl="Categoría"><Inp value={f.cat} onChange={e=>set("cat",e.target.value)}/></Fld></Row>
...
function AperturaCierre({sales,apts,svcs,staff,caja,setCaja,toast}){
  ...
  const calcTotalesVivo=()=>{
    ...
    const aptSales=apts.filter(a=>a.date===today&&(a.status==="completed"||a.paid)).map(a=>{
      const svc=svcs.find(x=>x.id===a.sid);return svc?.price||0;
    });
    const ef=daySales.filter(s=>s.pay==="efectivo").reduce((a,b)=>a+b.amount+(b.tip||0),0);
    const tc=daySales.filter(s=>s.pay==="tarjeta").reduce((a,b)=>a+b.amount+(b.tip||0),0);
    const nq=daySales.filter(s=>["nequi","daviplata","transferencia"].includes(s.pay)).reduce((a,b)=>a+b.amount+(b.tip||0),0);
    const propinas=daySales.reduce((a,b)=>a+(b.tip||0),0);
    const servicios=daySales.reduce((a,b)=>a+b.amount,0)+aptSales.reduce((a,b)=>a+b,0);
...
const SEED_MOVIMIENTOS = [
  {id:"m1",tipo:"gasto",cat:"Arriendo",desc:"Arriendo local Pinar del Río Mall",monto:2800000,fecha:D0(-5),pay:"transferencia",recurrente:true,archivado:false},
  {id:"m2",tipo:"gasto",cat:"Insumos",desc:"Compra extensiones Belleza Pro",monto:650000,fecha:D0(-3),pay:"tarjeta",recurrente:false,archivado:false},
  {id:"m3",tipo:"gasto",cat:"Servicios públicos",desc:"Energía + agua mes",monto:320000,fecha:D0(-8),pay:"nequi",recurrente:true,archivado:false},
  {id:"m4",tipo:"ingreso",cat:"Inversión socio",desc:"Aporte de capital Jimmy Rivera",monto:5000000,fecha:D0(-15),pay:"transferencia",recurrente:false,archivado:false},
  {id:"m5",tipo:"gasto",cat:"Marketing",desc:"Pauta TikTok Ads",monto:180000,fecha:D0(-2),pay:"tarjeta",recurrente:false,archivado:false},
];
...
function Finanzas({movimientos,setMovimientos,sales,staff,apts,svcs,toast}){
  ...
  const ventasMes=sales.filter(s=>new Date(s.date).getMonth()===filtroMes)
    .reduce((a,b)=>a+(b.total||b.amount*(1-(b.disc||0)/100)+(b.tip||0)),0);

  // Nómina del mes (salarios + comisiones calculados)
  const nominaMes=(staff||[]).reduce((acc,m)=>{
    const mSales=sales.filter(s=>s.tid===m.id&&new Date(s.date).getMonth()===filtroMes)
      .reduce((a,b)=>a+(b.total||b.amount),0);
    const comision=mSales*(m.comm/100);
    const neto=m.base+comision-m.base*0.08; // deducciones aprox
    return acc+neto;
  },0);
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",color:C.cream}}>
      ...
      <div style={{maxWidth:1140,margin:"0 auto",padding:"20px 16px 80px"}}>
        ...
        {tab==="dashboard"&&<Dashboard apts={apts} sales={sales} clients={clients} inv={inv} svcs={svcs} staff={staff} orders={orders}/>}
        {tab==="agenda"&&<Agenda apts={apts} setApts={setApts} clients={clients} svcs={svcs} staff={staff} toast={showToast} empresa={empresa}/>}
        {tab==="clientes"&&permisos.clientes&&<Clientes clients={clients} setClients={setClients} apts={apts} sales={sales} svcs={svcs} toast={showToast}/>}
